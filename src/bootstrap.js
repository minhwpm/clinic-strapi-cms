const fs = require("fs");
const { pages, globals, bookingFormSubmissions } = require("../data/data");
...
async function importBookingFormSubmissionData() {
  bookingFormSubmissions.forEach(async (submission) => {
    await createEntry("booking-form-submissions", submission);
  });
}
...
async function importSeedData() {
  ...
  await setPublicPermissions({
    global: ["find"],
    page: ["find", "findOne"],
    "booking-form-submission": ["create"],
  });
  ...
  await importBookingFormSubmissionData();
}
